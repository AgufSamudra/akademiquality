  ...
  return (
    <main className="min-h-dvh bg-white text-black">
      ...
      {false ? <section className="relative overflow-hidden bg-[#f8f8f7] px-5 py-20 md:px-10 md:py-28">
        ...
        <div className="relative mx-auto max-w-[1200px]">
          <div className="mx-auto max-w-3xl text-center">
            <p className="text-[14px] font-bold uppercase tracking-[0.16em] text-[#ae4169]">Pilihan Paket</p>
            <h2 className="mt-4 text-[38px] font-bold leading-tight tracking-tight text-black md:text-[54px]">Pilih format pelatihan yang paling sesuai.</h2>
            ...
            {pricingPlans.map(([title, price, originalPrice, unit, features, popular]) => (
              <article className={`relative flex min-h-[500px] flex-col overflow-hidden rounded-[26px] border p-6 shadow-[0_18px_55px_rgba(0,0,0,0.07)] transition-transform duration-300 hover:-translate-y-1 ${popular ? "border-black bg-[linear-gradient(145deg,#080808_0%,#281018_100%)] text-white shadow-[0_24px_70px_rgba(0,0,0,0.2)]" : "border-white/80 bg-white/75 text-black backdrop-blur-xl"}`} key={title}>
                ...
                <ul className={`mt-6 grid gap-3 text-[14px] leading-snug ${popular ? "text-white/78" : "text-black/65"}`}>
                  {features.map((feature) => <li className="flex gap-2.5" key={feature}><CheckCircle2 className={`shrink-0 ${popular ? "text-[#f3a3c0]" : "text-[#c84d7c]"}`} size={17} />{feature}</li>)}
                </ul>
                <a className={`${title === "InHouse Training Offline" ? "mt-10" : "mt-auto"} inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-[14px] font-semibold transition-transform hover:-translate-y-0.5 ${popular ? "bg-white text-black" : "bg-black text-white"}`} href={whatsappHref} rel="noreferrer" target="_blank">Konsultasikan Paket <ArrowRight size={16} /></a>
              </article>
            ))}
          </div>
        </div>
