...
export default async function TrainingDetailPage({ params }: Props) {
  ...
  return <main className="min-h-dvh bg-white text-black">
    ...
    <section className="relative overflow-hidden bg-[linear-gradient(135deg,#f5f3f4_0%,#ffffff_48%,#f8f8f7_100%)] px-5 py-16 md:px-10 md:py-24"><div className="absolute left-[-10%] top-[-20%] h-80 w-80 rounded-full bg-[#d64b7d]/10 blur-3xl" /><div className="relative mx-auto max-w-[1200px]"><div className="mx-auto max-w-3xl text-center"><p className="text-sm font-bold uppercase tracking-[.15em] text-[#b63161]">Pilihan paket</p><h2 className="mt-4 text-4xl font-bold leading-tight tracking-tight md:text-6xl">Pilih format pelatihan yang paling sesuai.</h2><p className="mx-auto mt-5 max-w-2xl text-[16px] leading-7 text-black/60">Tersedia pilihan kelas individu maupun paket untuk tim. Hubungi kami untuk penyesuaian kebutuhan organisasi Anda.</p></div><div className="mt-12 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{pricingPlans.map(([name, price, unit, features, popular]) => <article className={`relative flex min-h-[500px] flex-col rounded-[28px] border p-6 shadow-[0_15px_40px_rgba(0,0,0,.06)] ${popular ? "border-black bg-[linear-gradient(145deg,#080808_0%,#251018_100%)] text-white" : "border-black/8 bg-white text-black"}`} key={name}>{popular ? <span className="absolute right-5 top-5 rounded-full bg-[#df4d81] px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-white">Pilihan populer</span> : null}<h3 className="max-w-[180px] text-[21px] font-bold leading-tight">{name}</h3><div className={`mt-7 border-t pt-6 ${popular ? "border-white/15" : "border-black/12"}`}><p className="text-4xl font-bold tracking-tight">{price}</p><p className={`mt-1 text-sm ${popular ? "text-white/65" : "text-black/50"}`}>{unit}</p></div><ul className="mt-8 space-y-3">{features.map((feature) => <li className={`flex gap-2 text-[14px] leading-5 ${popular ? "text-white/80" : "text-black/60"}`} key={feature}><Check className="mt-0.5 shrink-0 text-[#df4d81]" size={17}/>{feature}</li>)}</ul><a className={`mt-auto inline-flex items-center justify-center gap-2 rounded-full px-4 py-3 text-sm font-bold ${popular ? "bg-white text-black" : "bg-black text-white"}`} href={whatsappHref} rel="noreferrer" target="_blank">Pilih paket <ChevronRight size={16}/></a></article>)}</div></div></section>
